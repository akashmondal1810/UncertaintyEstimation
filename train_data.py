...
#importing model generators for classifications
from training.train_bin import NNDropout, DeepArch_clf, XGBMulti, XGBRandom
#importing model generators for regression
...
if __name__ == '__main__':
	...
	if algo=='RandomXGB':
		...
		num_round = 500
		params = training_param

		training_param['tergetCol'] = tcol
		with open('training/training_strategy_xgbRndm.json', 'w') as fp:
			json.dump(training_param, fp)


		if prob_typ=='binary_classification':
			print('Detected problem type:: binary_classification')
			model = XGBRandom(x_train, y_train, params, num_round).get_model()


		elif prob_typ=='regression':
			print('Regression setup is not defined for the random XGBoost method')
			pass
