...
class DeepEn_reg():
    ...
    def fit_model(self,dropout_prob=0.10, n_epochs=10, tau=1.0, batch_size=1024, lengthscale=1e-2, n_hidden=[100,150, 100]):
        ...
        print('Fitting NN for Deep Ensemble...')

        model_de = self.architecture(n_hidden=n_hidden, input_dim=input_dim, dropout_prob=dropout_prob, reg=reg)
        model_de.compile(optimizer='adam', loss=self.gaussian_nll, metrics=['mean_squared_error'])
        ...
        print('Training complete with Running Time: ', self.running_time)
