...
class NNDropout_multi():
    """
            Constructor for the class implementing a neural network architecture for multiclass classification
            @param mc             True in case of MC Dropout architecture and False for standard
                                  nurel network.
            @param activn_fn      Activation function to be used
            
            @param activn_fn      No of labels or classes
    """
    def __init__(self, mc, actvn, num_class):
        self.mc = mc
    ...
    def architecture(self, n_hidden, input_dim, dropout_prob, reg):
        ...
        inter = tf.keras.layers.Dropout(dropout_prob)(inter, training=self.mc)
        outputs = tf.keras.layers.Dense(self.num_class, activation='softmax')(inter) 
    ...
    def model_runner(self, X_train, y_train, dropout_prob=0.10, n_epochs=10, tau=1.0, batch_size=1024, 
        ...
        model_mc_dropout.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
